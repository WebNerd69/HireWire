...
const Login = () => {
  ...
  return (
    <div className='w-[100vw] h-[100vh] flex justify-center items-center relative overflow-hidden'>
      <span className='sarina-regular text-[160vh] absolute md:left-14 left-3 text-[#686df8] -z-10'>h</span>
      <div className='md:w-[40%] w-[90%] h-[50%] bg-[#ffffff90] rounded-xl shadow-xl flex flex-col gap-y-5 justify-center items-center z-10 backdrop-blur-xl poppins-medium'>
        ...
        {currentState === 'login' && (<p className='absolute md:bottom-10 bottom-5 md:left-14 left-5 text-sm text-gray-500' >Don't have an account? <span className='text-[#686df8] cursor-pointer ' onClick={() => setCurrentState('signup')}>Sign up</span></p>)}
        {currentState === 'signup' && (<p className='absolute md:bottom-10 bottom-5 md:left-14 left-5 text-sm text-gray-500' >Already have an account? <span className='text-[#686df8] cursor-pointer ' onClick={() => setCurrentState('login')}>Login</span></p>)}
      </div>
    </div>
  )
}
